...
- A follow-up whose re-measure finished could go unreported: the number was posted only after the push, and the push folded the ledger into the sealed commit with an amend that git refuses when the session had committed its change itself and the ledger row did not move. The measured number is now posted as soon as it is known, once, whatever becomes of the push; and a seal with nothing to fold is pushed as the session's own commit.
...
- `outerloop init --github-app` — the **recommended** auth path: one-click
  ...
  (`setup.outerloop.science`, in `web/index.html`) that carries the manifest

...
built. The question: try Outerloop on a second cluster while 0.2.0 soaks
(Empire AI Alpha, NERSC Perlmutter, possibly ALCF Polaris), and say what
...
runtime shows in two: `dispatch.py` and `harness.py`, both of which build
`apptainer exec --containall --cleanenv [--nv] --bind ...` by hand.

## The three tiers

**Tier 1, independent deployments.** Each cluster runs its own kernel on its
own target, or on the same target with disjoint agent slots. The only
coordination is what GitHub already provides: a second kernel reads the
...
- Claims and publishes are already safe across kernels: issue claims are
  GitHub comment markers, the ledger and board are pushed with an
  expected-head guard, and PR merges go through GitHub. Self-initiated
...
so tier 1 should run with the current code. Two things to settle on the
...
and the tick needs a CPU partition it may occupy for six hours at a time.
If our subaccount has no CPU partition, see the tick-host decision below.
...
hard-codes `apptainer exec` in two places and Perlmutter offers Shifter and
podman-hpc instead (one `ContainerRuntime` with an apptainer and a shifter
implementation; the image needs an OCI publication next to the `.sif`); and
...
2. Install. A venv from PyPI (`outerloop-science==0.2.0rc2`) or from main;
   `outerloop init` writes `~/.config/outerloop/.env` with the subaccount,
   the CPU partition, the GPU partition, the image path and the App file.

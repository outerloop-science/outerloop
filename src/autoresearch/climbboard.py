...
def render_html(
    ...
    return (
        ...
        "padding:.45rem .7rem;font-size:.85rem;color:var(--muted);min-width:14rem}\n"
        ...
        ".run .mlabel{font-size:.65rem;color:var(--muted);white-space:nowrap}\n"
        ".run .meter{display:flex;gap:2px;width:56px}\n"
        ...
        "    meta.title = 'time in this state (since the last transition: park,'\n"
        "      + ' submit, wake) · GPU-hours used/budget'\n"
        "      + ' · experiment jobs finished/launched';\n"
        "    meta.textContent = t + gpu + exp;\n"
        ...
        "      // labeled in place: a bare dash meter reads as decoration\n"
        "      const lbl = document.createElement('span'); lbl.className = 'mlabel';\n"
        "      lbl.textContent = 'depth ' + r.launches_used + '/' + r.depth_k;\n"
        "      row.append(lbl);\n"

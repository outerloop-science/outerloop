...
def render_html(
    ...
    return (
        ...
        "    card2.append(lx, ly);\n"
        "    el.append(h3, card2);\n"
        "    // one legend entry per agent; clicking toggles all of that\n"
        "    // agent's lines (solid box = at least one line visible)\n"
        "    const agentLegend = {labels: {generateLabels: (chart) => {\n"
        "      const seen = [];\n"
        "      withCurve.forEach((r, i) => {\n"
        "        let g = seen.find(x => x.agent === r.agent);\n"
        "        if (!g) { g = {agent: r.agent, idxs: []}; seen.push(g); }\n"
        "        g.idxs.push(i);\n"
        "      });\n"
        "      return seen.map(g => {\n"
        "        const shown = g.idxs.some(i => chart.isDatasetVisible(i));\n"
        "        const color = agentColor(g.agent);\n"
        "        return {text: g.agent, datasetIndex: g.idxs[0], hidden: false,\n"
        "          fillStyle: shown ? color : 'rgba(0,0,0,0)',\n"
        "          strokeStyle: color, lineWidth: 2, fontColor: css('--muted')};\n"
        "      });\n"
        "    }},\n"
        "    onClick: (e, item, legend) => {\n"
        "      const c = legend.chart;\n"
        "      const idxs = withCurve\n"
        "        .map((r, i) => (r.agent === item.text ? i : -1))\n"
        "        .filter(i => i >= 0);\n"
        "      const anyOn = idxs.some(i => c.isDatasetVisible(i));\n"
        "      idxs.forEach(i => c.setDatasetVisibility(i, !anyOn));\n"
        "      c.update();\n"
        "    }};\n"
        "    let chart2;\n"
        "    const draw2 = () => {\n"
        ...
        "      const bar = cell('bar');\n"
        "      const over = timeFrac(r.eval_minutes) >= 1;\n"
        "      bar.append(layer(timeFrac(r.eval_minutes),\n"
        "        over ? css('--base') : agentColor(r.agent)));\n"
        "      addRow('gate', bar,\n"
